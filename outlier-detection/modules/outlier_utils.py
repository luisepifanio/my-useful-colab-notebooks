...
def prophet_fit(df, prophet_model, today_index, lookback_days=None, predict_days=21):
    ...
    if not lookback_days:
        print(
            "Use the data from {} to {} ({} days)".format(
                df["ds"][0], df["ds"][today_index - 1], today_index
            )
        )
    else:
        ...
        print(
            "Use the data from {} to {} ({} days)".format(
                df["ds"][today_index - lookback_days],
                df["ds"][today_index - 1],
                lookback_days,
            )
        )
    print(
        "Predict {} to {} ({} days)".format(
            df["ds"][today_index], df["ds"][today_index + predict_days - 1], predict_days
        )
    )

    # fit the model
    prophet_model.fit(pd.DataFrame({"ds": baseline_ts.values, "y": baseline_y.values}))
...
def prophet_plot(
    df, fig, today_index, lookback_days=None, predict_days=21, outliers=list()
):
    """
    ...
    ax.plot(x_pydatetime[start:end], df.y[start:end], color="orange", label="Actual")
    ...
    ax.text(
        x_pydatetime[int((start + today_index) / 2)],
        ymin + (ymax - ymin) / 20,
        "Baseline area",
    )
    ax.text(
        x_pydatetime[int((today_index * 2 + predict_days) / 2)],
        ymin + (ymax - ymin) / 20,
        "Prediction area",
    )
...
def get_outliers(df, forecast, today_index, predict_days=21):
    ...
    for i in range(df_pred.shape[0]):
        ...
        if actual_value < df_pred["lower_y"][i] or actual_value > df_pred["upper_y"][i]:
            ...
            print("=====")
            print(
                "actual value {} fall outside of the prediction interval".format(
                    actual_value
                )
            )
            print(
                "interval: {} to {}".format(df_pred["lower_y"][i], df_pred["upper_y"][i])
            )
